...
extension OperationsManager {
    ...
    public func refreshServices(projectName: String?) -> Void {
        ...
        for (serviceName, _) in prevServicesURLs {
            if let (_, serviceConfig) = self.resourceManager.decodeServiceConfig(pName: projectHandle, serviceName: serviceName) {
        ...
        if fileManager.fileExists(atPath: servicesURL.path) {
            do {
                ...
                for url in directoryItems {
                    guard let service = self.resourceManager.fetchServiceFromURL(serviceURL: url) else {
                        abortMessage(msg: "Refresh services")
                        return
                    }
                ...
                for (name, service) in updatedServices {
                    if let serviceConfigURL = self.resourceManager.encodeServiceConfig(pName: projectHandle, configData: service) {
