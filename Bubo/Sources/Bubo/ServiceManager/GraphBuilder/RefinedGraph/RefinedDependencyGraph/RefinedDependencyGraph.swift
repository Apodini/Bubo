...
open class RefinedDependencyGraph<V: Equatable & Codable>: Graph {
    public var vertices: [V] = [V]()
    public var edges: [[RefinedDependencyEdge]] = [[RefinedDependencyEdge]]() //adjacency lists
    public var groupMap: [Int:RawDependencyGraph<Node>] = [Int:RawDependencyGraph<Node>]()
    ...
    ///
    /// - parameter group: The group of clustered nodes
    /// - returns: The groupID of the newly added group
    public func addGroup(group: RawDependencyGraph<Node>) -> Int {
        let groupID = groupMap.count + 1
    ...
    ///                       Default is false.
    public func addEdge(_ e: RefinedDependencyEdge, directed: Bool) {
        edges[e.u].append(e)
        if !directed && e.u != e.v {
...
extension Graph where E == RefinedDependencyEdge {
    ...
    public func addEdge(fromIndex: Int, toIndex: Int, directed: Bool = false, role: [EdgeRole]) {
        addEdge(RefinedDependencyEdge(u: fromIndex, v: toIndex, directed: directed, roles: role), directed: directed)
    }
    ...
    public func addEdge(from: V, to: V, directed: Bool = false, role: [EdgeRole]) {
        if let u = indexOfVertex(from), let v = indexOfVertex(to) {
            addEdge(RefinedDependencyEdge(u: u, v: v, directed: directed, roles: role), directed: directed)
        }
    }
